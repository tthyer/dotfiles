...
# CLAUDE.md is composed rather than linked: both repos contribute global memory, and a
# symlink can only point at one of them. Keeping the overlay's half out of the public repo
# is the point — anything naming an employer, a colleague or a workflow belongs over there.
{
  echo "# Generated by dotfiles/setup/agents-setup.sh — edit the imported files, not this one."
  echo
  echo "@$DOTFILES_DIR/config/claude/CLAUDE.md"
  # A plain `[[ ... ]] && echo` here would leave the block's status at 1 on a machine with
  # no overlay, and `set -e` would abort the whole run.
  if [[ -f "$OVERLAY_DIR/config/claude/CLAUDE.md" ]]; then
    echo "@$OVERLAY_DIR/config/claude/CLAUDE.md"
  fi
